...
def packet_loss_counter(pkt):
    global sent_packets, lost_packets
    if pkt.haslayer(ICMP) and pkt[ICMP].type == 8:  # ICMP Echo Request
        sent_packets += 1
    elif pkt.haslayer(ICMP) and pkt[ICMP].type == 0:  # ICMP Echo Reply
        sent_packets += 1
    elif pkt.haslayer(ICMP) and pkt[ICMP].type == 3:  # ICMP Destination Unreachable
        lost_packets += 1
...
def main():
    ...
    sniff(prn=packet_loss_counter, timeout=runtime)
    ...
    sys.stdout.write("\rGesamte Uptime: {:.2f}% | Laufzeit: {:.2f}s | Gesendete Pakete: {} | Verlorene Pakete: {}\n".format(uptime_percent, runtime, sent_packets, lost_packets))
    sys.stdout.flush()
